...
class CavityResonator():
    ...
    def to_gpu(self, recursive=True):
        ...
        self.Ncav = cp.asarray(self.Ncav)
        self.Rs_per_cavity = cp.asarray(self.Rs_per_cavity)
        self.Q = cp.asarray(self.Q)
        self.QL = cp.asarray(self.QL)
        self.beta = cp.asarray(self.beta)        
        self.detune = cp.asarray(self.detune)
        self.fr = cp.asarray(self.fr)        
        self.wr = cp.asarray(self.wr)  
        self.psi = cp.asarray(self.psi)         
        self.Vc = cp.asarray(self.Vc)
        self.theta = cp.asarray(self.theta)
        
        self.beam_phasor = cp.asarray(self.beam_phasor)
        self.beam_phasor_record = cp.asarray(self.beam_phasor_record)
        self.Vg = cp.asarray(self.Vg)
        self.theta_g = cp.asarray(self.theta_g)
        self.Vgr = cp.asarray(self.Vgr)
        self.theta_gr = cp.asarray(self.theta_gr)
        self.Pg = cp.asarray(self.Pg)
        self.n_bin = cp.asarray(self.n_bin)
        self.distance = cp.asarray(self.distance)
        self.valid_bunch_index = cp.asarray(self.valid_bunch_index)       
        self.nturn = cp.asarray(self.nturn)
        self.filling_time = cp.asarray(self.filling_time) 
        self.tracking = cp.asarray(self.tracking)         
